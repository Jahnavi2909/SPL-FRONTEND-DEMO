// components/franchise/FranchiseSidebar.jsx
import { NavLink, useNavigate } from "react-router-dom";
import { X } from "lucide-react";
import { franchiseSidebarSections } from "../../utils/dashboardData";

export default function FranchiseSidebar({ mobileOpen, onClose }) {
  const navigate = useNavigate();

  const handleLogout = () => {
    localStorage.removeItem("spl_franchise_logged_in");
        ...
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-5 lg:block">
          <div>
            <div className="font-heading text-3xl tracking-[0.16em] text-green-400">
              SP<span className="text-yellow-400">L</span>
            </div>

            <div className="mt-5 flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-full border border-green-400/20 bg-green-500/10 text-lg text-green-400">
                🏏
              </div>
