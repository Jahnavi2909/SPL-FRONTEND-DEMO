...
import AdminLiveMatchPanel from "../../components/dashboard/AdminLiveMatchPanel";
...
function getTopPerformerLabel(type) {
  return type === "batting" ? "Top Batter" : "Top Bowler";
}

function getTopPerformerValue(player) {
  return player?.type === "batting" ? player.score : player.wickets;
}

function getTopPerformerMeta(player) {
  if (player?.type === "batting") {
    return `Avg ${player.average.toFixed(1)} | ${player.matches} matches`;
  }

  return `Eco ${player.economy.toFixed(1)} | ${player.matches} matches`;
}

function getMatchTimestamp(match) {
  return new Date(match?.raw?.match_date || match?.raw?.created_at || 0).getTime();
}
...
export default function AdminDashboard() {
  ...
  const formattedMatches = useMemo(() => {
    return fixtures
      ...
      .sort((firstMatch, secondMatch) => getMatchTimestamp(firstMatch) - getMatchTimestamp(secondMatch));
  }, [fixtures, teams, venues]);

  const liveMatches = useMemo(() => {
    return formattedMatches.filter((match) => match.status === "Live");
  }, [formattedMatches]);
  ...
  const nextMatch = useMemo(() => {
    return (
      formattedMatches.find((match) =>
        ["Pending", "Upcoming", "Draft"].includes(match.status)
      ) || null
    );
  }, [formattedMatches]);

  const liveMatchLoading = fixturesLoading || teamsLoading || venuesLoading;
  const liveMatchError = fixturesError || teamsError || venuesError || "";
    ...
    <div className="space-y-5 bg-white">
      ...
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-[1.7fr_1fr]">
        ...
        <div className="space-y-5">
          ...
          <DashboardPanel title="Live Now" bodyClassName="p-0">
            <AdminLiveMatchPanel
              liveMatch={liveMatch}
              nextMatch={nextMatch}
      ...
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-2">
        ...
        <DashboardPanel title="Top Performers">
          {topPerformersLoading ? (
            ...
            <div className="space-y-1">
              {topPerformerCards.map((player) => {
                const avatarColor = player.type === "batting" ? "gold" : "blue";
                const badgeColor = player.type === "batting" ? "orange" : "blue";
                const logoUrl = getMediaUrl(player.logo);

                return (
                  <div
                    key={`${player.type}-${player.playerId || player.name}`}
                    className="flex items-center gap-4 border-b border-slate-100 py-3 last:border-b-0"
                  >
                    <div
                      className={`flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full border text-[12px] font-bold ${getAvatarColorClasses(
                        avatarColor
                      )}`}
                    >
                      {logoUrl ? (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          label={getTopPerformerLabel(player.type)}
                          color={badgeColor}
                        />
                      </div>
